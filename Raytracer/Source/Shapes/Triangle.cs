...
namespace Raytracer.Source.Shapes
{
    ...
    public struct Triangle : IRaytracable
    {
        ...
        public Triangle(Vector3 leftFront, Vector3 leftBack, Vector3 rightBack, Vector3 rightFront, Vector3 topCenter, IMaterial material)
        {
            _piramidCoords = new Vector3[4][];
            _piramidCoords[0] = new Vector3[3];
            _piramidCoords[0][0] =leftFront;
            _piramidCoords[0][1] = leftBack;
            _piramidCoords[0][2] = topCenter;
    
            _piramidCoords[1] = new Vector3[3];
            _piramidCoords[1][0] = leftBack;
            _piramidCoords[1][1] = rightBack;
            _piramidCoords[1][2] = topCenter;
    
            _piramidCoords[2] = new Vector3[3];
            _piramidCoords[2][0] = rightBack;
            _piramidCoords[2][1] = rightFront;
            _piramidCoords[2][2] = topCenter;
            
            _piramidCoords[3] = new Vector3[3];
            _piramidCoords[3][0] = rightFront;
            _piramidCoords[3][1] = leftFront;
            _piramidCoords[3][2] = topCenter;
    
            Material = material;

            var rightTriangleCorner = rightBack;
            rightTriangleCorner.Y += topCenter.Y-leftFront.Y;
            var dimensions = rightTriangleCorner - leftFront;

            Bounds = new CubeBound(leftFront.X, leftFront.Y, leftFront.Z, dimensions.X, dimensions.Y,
                dimensions.Z);
        }
